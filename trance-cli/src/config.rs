...
pub fn handle_config(client: &TranceClient, args: &[String]) -> Result<(), String> {
    ...
    match args[0].as_str() {
        "list" => {
            ...
            println!("active_saver:      {}", if status.active_saver.is_empty() { "random" } else { &status.active_saver });
        ...
        "get" => {
            ...
            match key {
                ...
                "active_saver" | "saver" => println!("{}", if status.active_saver.is_empty() { "random" } else { &status.active_saver }),
                "gpu_enabled" | "gpu" => println!("{}", status.gpu_enabled),
                "show_fps_overlay" | "fps" => println!("{}", status.show_fps_overlay),
                "render_scale" | "scale" => println!("{}", if status.render_scale.is_empty() { "default" } else { &status.render_scale }),
        ...
        "set" => {
            ...
            match key {
                "idle_enabled" | "enabled" => {
                    let b = val.parse::<bool>().map_err(|_| "value must be true or false")?;
                    if b { client.enable() } else { client.disable() }.map_err(|e| e.to_string())?;
                }
                "idle_timeout_mins" | "timeout" => {
                    let n = val.parse::<u32>().map_err(|_| "value must be an integer (1–240)")?;
                    if !(1..=240).contains(&n) {
                ...
                "active_saver" | "saver" => {
                    let name = if val == "random" || val == "none" { "" } else { val };
                ...
                "render_scale" | "scale" => {
                    let scale = if val == "default" {
                        ...
                        val.parse::<f32>().map_err(|_| "value must be between 0.25 and 1.0, or 'default'")?
                    };
